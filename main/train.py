...
from lib.model.generator.MSPF_Net import SSNet,MSPFNet,MSEFNet,OSPFNet,MSFFNet,MSCFNet
...
from rm_cloud.single_model_methods.mspf_trainer import MSPFTrainer
...
def train():
    ...
    if args.model.arch == 'MSPF':
        if args.name == 'MSPF':
            ...
            adm_optimizer = get_adam_optimizer(args, model)
            msf_net = MSPFTrainer(model, adm_optimizer)
            msf_net.train_multi_scale(args, train_loader, test_loader, args.start_epoch, args.end_epoch)
        ...
        if args.name == 'OSPF':
            ...
            adm_optimizer = get_adam_optimizer(args, model)
            msf_net = MSPFTrainer(model, adm_optimizer)
            msf_net.train_same_scale(args, train_loader, test_loader, args.start_epoch, args.end_epoch)
        ...
        if args.name == 'SS':
            ...
            adm_optimizer = get_adam_optimizer(args, model)
            msf_net = MSPFTrainer(model, adm_optimizer)
            msf_net.train_single_scale(args, train_loader, test_loader, args.start_epoch, args.end_epoch)
        ...
        # adm_optimizer = get_adam_optimizer(args, model)
        # msf_net = MSPFTrainer(model, adm_optimizer)
        # msf_net.train_same_multi(args, train_loader, test_loader, args.start_epoch, args.end_epoch)
