...
class Pix2pixhd(nn.Module):
    ...
    def forward(self, x1):
        ...
        downsample_1 = F.relu(self.downsample1(x))
...
class Pix2pixhd(nn.Module):
    def __init__(self):
        super(Pix2pixhd, self).__init__()
        self.input = conv(9, 16,s=1, k=3, p=1)
        self.bn = bn(16)
        self.downsample = conv(16,16,s=2,k=3,p=1)
        self.block = Block_skip(16)
        self.output1 = deconv(32, 16, s=2, k=4, p=1)
        self.output2 = conv(16, 9, s=1, k=3, p=1)

    def forward(self, x):
        x1 = self.input(x)
        x1 = F.leaky_relu(self.bn(x1),0.2,True)
        downsample = self.downsample(x1)
        downsample = F.leaky_relu(self.bn(downsample),0.2,True)
        l = self.block(downsample)
